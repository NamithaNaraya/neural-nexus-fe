    ...
    return (
        <div className={`fixed ${isExpanded ? 'inset-0' : 'bottom-6 right-6'} z-[160] flex flex-col items-end pointer-events-none transition-all duration-500`}>
            <AnimatePresence>
                {isOpen && (
                    <motion.div
                        ...
                        className={`pointer-events-auto bg-white/95 backdrop-blur-3xl border border-emerald-500/10 shadow-[0_20px_60px_rgba(0,0,0,0.1)] overflow-hidden flex flex-col transition-all duration-500 ease-in-out ${isExpanded ? 'w-full h-full rounded-none m-0' : 'w-[500px] h-[750px] rounded-[2.5rem] mb-4'
                            }`}
                    >
                        {/* Header */}
                        <div className="px-6 py-5 border-b border-emerald-500/10 flex items-center justify-between bg-emerald-500/5">
                            <div className="flex items-center gap-3">
                                <div className="p-2.5 rounded-2xl bg-emerald-500/10 text-emerald-600 ring-1 ring-emerald-500/20">
                                    <Brain size={24} />
                                </div>
                                <div>
                        ...
                        {/* Content Area */}
                        <div ref={scrollRef} className="flex-1 overflow-y-auto p-6 space-y-8 scrollbar-thin scrollbar-thumb-emerald-500/10">
                            ...
                            {sessionMessages.length === 0 && !isProcessing && (
                                <div className="h-full flex flex-col items-center justify-center text-center px-10 pt-20">
                                    ...
                                    <h4 className="text-xl font-bold text-slate-900 mb-3">Neural Matrix Ready</h4>
                            ...
                            {sessionMessages.map((msg, i) => (
                                <div key={msg.id} className={`flex gap-4 ${msg.role === 'user' ? 'flex-row-reverse' : 'flex-row'}`}>
                                    <div className={`mt-1 flex-shrink-0 w-9 h-9 rounded-2xl flex items-center justify-center border shadow-sm ${msg.role === 'user' ? 'bg-slate-100 border-slate-200 text-slate-600' : 'bg-emerald-500/10 border-emerald-500/20 text-emerald-600'
                                        }`}>
                                        ...
                                        {msg.isReasoning && msg.reasoningOutcome && (
                                            <div className="mt-4 w-full space-y-4">
                                                ...
                                                {showOutcomeForm !== msg.id ? (
                                                    ...
                                                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="p-4 rounded-2xl bg-slate-50 border border-slate-200 space-y-3 shadow-sm">
                                                        ...
                                                        <textarea
                                                            className="w-full bg-white border border-slate-200 rounded-xl p-3 text-xs text-slate-800 h-20 outline-none focus:border-emerald-500/50"
                            ...
                            {isProcessing && (
                                <div className="space-y-6">
                                    <div className="flex gap-4">
                                        ...
                                        <div className="px-5 py-3.5 rounded-[1.5rem] rounded-tl-none bg-slate-50 border border-slate-100 flex items-center gap-3">
                        ...
                        {/* Footer Input */}
                        <div className="p-6 border-t border-emerald-500/10 bg-emerald-500/5">
                            <form onSubmit={handleSend} className="relative">
                                <input
                                    ...
                                    className="w-full bg-white border border-slate-200 rounded-2xl px-5 py-4 pr-14 text-sm text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-emerald-500/20 transition-all shadow-sm"
            ...
            <motion.button
                ...
                className={`pointer-events-auto w-16 h-16 rounded-[1.5rem] flex items-center justify-center shadow-[0_20px_40px_rgba(0,0,0,0.1)] transition-all duration-500 relative group overflow-hidden ${isOpen ? 'bg-white text-emerald-600 border border-emerald-100 rotate-90' : 'bg-white text-emerald-600 border border-emerald-100'
                    }`}
            >
